...
use definiciones
implicit none
integer(4)::computing_time,computing_time_final,clock_rate,clock_max,index_1,index_2,indice


integer(4)::ke,i_time,i
...
!i_print=1000
n_0=1.5e7!in cells/ml
g=980.0d0!in cm/s^2
delta_rho=0.05!delta_rho/rho
v_s=0.14d0*78d-4!58.0d-4;
l=1.0d0
r_cell=5.0d-4!in cm
pi=4.0d0*atan(1.0d0)
nu=1.0d-2
...
gr=n_0*g*delta_rho*(4.0d0/3.0d0)*pi*(r_cell)**3*(l**3.0d0)/nu**2
sc=nu/diff_cell
...
if (indice<=index_1)then

w_photo=w_photo_1
u_photo=u_photo_1

else if (indice>index_1 .and. indice<=2*index_1)then
...
if (indice==2*index_1)then
indice=0
end if

if(i_write==10000)then
suma_n=sum(n_new)/((nx_i+1)*(nz_i+1))
write(*,*)i_time,i_write,indice
write(*,'(2e17.9,1x)')dabs(suma_n-1.0d0),dabs(sum(n_p_new)/((nx_i+1)*(nz_i+1)))
...
n_passive(1:nx_i+1,1:nz_i+1)=n_p_new(1:nx_i+1,1:nz_i+1)
ke=1


if(i_time>=nt_i+2)then
exit
end if
...
allocate(u_new(nx_i+1,nz_i+1),w_new(nx_i+1,nz_i+1),psi_new(nx_i+1,nz_i+1),omega_new(nx_i+1,nz_i+1),&
        n_new(nx_i+1,nz_i+1))
allocate(n_passive(nx_i+1,nz_i+1),n_p_aprox(nx_i+1,nz_i+1),n_p_aprox_2(nx_i+1,nz_i+1),&
        n_p_new(nx_i+1,nz_i+1))
...
allocate(u_photo_1(nx_i+1,nz_i+1),w_photo_1(nx_i+1,nz_i+1),intensity_1(nx_i+1,nz_i+1))
allocate(u_photo_2(nx_i+1,nz_i+1),w_photo_2(nx_i+1,nz_i+1),intensity_2(nx_i+1,nz_i+1))
...
x_foco_1=x_foco-0.15d0
x_foco_2=x_foco+0.15d0
do i=1,nx_i+1
    do j=1,nz_i+1

        intensity_1(i,j)=exp(-((x(i)-x_foco_1)**2+(z(j)-z_foco)**2)/(2.0d0*(sigma_l)**2))
        u_photo_1(i,j)=alpha_phot*sqrt((x(i)-x_foco_1)**2+(z(j)-z_foco)**2)/(519d-4)*&
                        (-exp(0.5d0))/sigma_l*(x(i)-x_foco_1)*exp(-((x(i)-x_foco_1)**2&
                    +(z(j)-z_foco)**2)/(2.0d0*sigma_l**2))
        w_photo_1(i,j)=-alpha_phot*sqrt((x(i)-x_foco_1)**2+(z(j)-z_foco)**2)/(519d-4)*&
                        exp(0.5d0)/sigma_l*(z(j)-z_foco)*exp(-((x(i)-x_foco_1)**2&
...
do i=1,nx_i+1
    do j=1,nz_i+1
        ...
        intensity_2(i,j)=exp(-((x(i)-x_foco_2)**2+(z(j)-z_foco)**2)/(2.0d0*(sigma_l)**2))
        u_photo_2(i,j)=alpha_phot*sqrt((x(i)-x_foco_2)**2+(z(j)-z_foco)**2)/(519d-4)*&
                        (-exp(0.5d0))/sigma_l*(x(i)-x_foco_2)*exp(-((x(i)-x_foco_2)**2&
                    +(z(j)-z_foco)**2)/(2.0d0*sigma_l**2))
        w_photo_2(i,j)=-alpha_phot*sqrt((x(i)-x_foco_2)**2+(z(j)-z_foco)**2)/(519d-4)*&
                        exp(0.5d0)/sigma_l*(z(j)-z_foco)*exp(-((x(i)-x_foco_2)**2&
                    +(z(j)-z_foco)**2)/(2.0*sigma_l**2))

        end do
end do

!open(unit=15,file='intensity_z_5d_1.dat',status='unknown')
!open(unit=16,file='u_photo_z_5d_1.dat',status='unknown')
!open(unit=17,file='w_photo_z_5d_1.dat',status='unknown')
...
do i=1,nx_i+1
    do j=1,nz_i+1
        n_passive(i,j)=0.5d0*(1+tanh((x(i)-x_foco)/5.0d-4))
!        n_old(i,j)=1.0d0
    end do
end do
...
n_aprox(i,j)=n_old(i,j)+delta_t/2.0d0*k_n(1,1)
n_p_aprox(i,j)=n_passive(i,j)+delta_t/2.0d0*k_n_p(1,1)
...
n_aprox_1(i,j)=n_old(i,j)+delta_t*(-k_n(1,1)+2.0d0*k_n(2,1))
n_p_aprox_2(i,j)=n_passive(i,j)+delta_t*(-k_n_p(1,1)+2.0d0*k_n_p(2,1))
    end do
...
open(unit=21,file='./data_psi_blinking/psi_0'//trim(counter)//'.dat',status='unknown')
open(unit=22,file='./data_n_blinking/n_0'//trim(counter)//'.dat',status='unknown')
open(unit=23,file='./data_u_blinking/u_0'//trim(counter)//'.dat',status='unknown')
open(unit=24,file='./data_w_blinking/w_0'//trim(counter)//'.dat',status='unknown')
open(unit=25,file='./data_vort_blinking/omega_0'//trim(counter)//'.dat',status='unknown')
open(unit=26,file='./data_n_p_blinking/n_p_0'//trim(counter)//'.dat',status='unknown')

write(21,'(129e17.9,1x)')(psi_old(i,1:nz_i+1),i=1,nx_i+1)
write(22,'(129e17.9,1x)')(n_old(i,1:nz_i+1),i=1,nx_i+1)
write(23,'(129e17.9,1x)')(u_old(i,1:nz_i+1),i=1,nx_i+1)
write(24,'(129e17.9,1x)')(w_old(i,1:nz_i+1),i=1,nx_i+1)
write(25,'(129e17.9,1x)')(omega_old(i,1:nz_i+1),i=1,nx_i+1)
write(26,'(129e17.9,1x)')(n_passive(i,1:nz_i+1),i=1,nx_i+1)
